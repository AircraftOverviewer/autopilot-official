module.exports = {
  ...
  parameters: {
    ...
    2: { name: '<mass>', description: '- Input the weight on Earth of an object you want to compare in kg' }
  },
  execute (Discord, message, args, client) {
    if (!args[0]) {
      const help = require('./help.js');
      help.execute(Discord, message, ['gravity'], client);
      return;
    }
    ...
    const resultsEmbed = new Discord.MessageEmbed()
      ...
      .setTitle('Gravity Calculation - Result')
      .addFields(
        { name: 'Gravity', value: `${planet.gravity} m.s⁻²` },
        { name: 'Weight on Earth', value: `${parseFloat(result).toFixed(2)} kg` },
