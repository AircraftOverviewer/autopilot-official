module.exports = {
  name: 'suvat',
  shortcut: ['motion'],
  description: 'Calculate motion in a given direction when acceleration is constant using a minimum of 3 known values from the Basic Motion Equation (SUVAT) to find the remaining unknowns to 2 decimal places',
  usage: 'ap suvat [values](<S> <U> <V> <A> <T>)',
  parameters: {
  ...
  execute (Discord, message, args, client) {
    ...
    const suvatEmbed = new Discord.MessageEmbed()
      ...
      .addFields(
        ...
        { name: 'Initial Velocity (U)', value: `${parseFloat(U).toFixed(2)} m.s⁻¹` },
        { name: 'Final Velocity (V)', value: `${parseFloat(V).toFixed(2)} m.s⁻¹` },
        { name: 'Acceleration (A)', value: `${parseFloat(A).toFixed(2)} m.s⁻²` },
        { name: 'Time (T)', value: `${parseFloat(T).toFixed(2)} s` }
      );
